...
export const loginDataService = async (email, password) => {
  ...
  await User.findByIdAndUpdate(foundUser.id, { token: generatedToken });
...
export const updateUserUserDataService = async (userId, params) => {
  ...
  if (!foundUser) throw HttpError(401, "User not found");
...
export const regenerateTokenDataService = async (userId) => {
  ...
  await User.findByIdAndUpdate(userId, { token: generatedToken });
  return generatedToken;
};
